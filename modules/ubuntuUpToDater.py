...
def get_latest_ubuntu_version():
    ...
    if match:
        ...
        if (get_installed_ubuntu_version() == latest_version):
            return (f'Ubuntu is up to date. Installed version is {latest_version}.', False)
        else:
            return (f'Ubuntu is not up to date. Installed version is {get_installed_ubuntu_version()}. Latest version is {latest_version}.', True)
    else:
        return None
